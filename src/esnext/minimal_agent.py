...
from .context_compaction import handoff_input, is_context_limit_error, maybe_compact_session
...
def start_agent_session(
    ...
    include_lifecycle_tools: bool = True, disable_task: bool = False, event_bus: EventBus | None = None, event_layer: str = "L3",
    event_context: dict[str, object] | None = None,
) -> AgentSession:
    session = create_agent_session(
        cwd=cwd, model=model, max_steps=max_steps, system_prompt=system_prompt, log_path=log_path,
        tools=tools, include_lifecycle_tools=include_lifecycle_tools, disable_task=disable_task,
    )
...
def create_agent_session(
    *, cwd: str | Path | None = None, model: str = MODEL, max_steps: int = 0, system_prompt: str = SYS,
    log_path: str | Path | None = None, tools: list[Any] | None = None, include_lifecycle_tools: bool = True, disable_task: bool = False,
) -> AgentSession:
    ...
    return AgentSession(
        info, system_prompt, MemorySaver(), list(tools or []),
        include_lifecycle_tools=include_lifecycle_tools, disable_task=disable_task, process_registry=CommandProcessRegistry(),
    )
...
def resume_agent_session(
    ...
    event_bus: EventBus | None = None, event_layer: str = "L3", event_context: dict[str, object] | None = None, allow_compaction_retry: bool = True,
) -> AgentRunResult:
    trace = RunTrace(session.info.snapshot())
    event_data = dict(event_context or {})
    compacted = maybe_compact_session(session, event_bus=event_bus, event_layer=event_layer, event_data=event_data)
    is_waiting_resume = bool(not compacted and not start and session.resume_mode == "interrupt" and session.last_result and session.last_result.status == "waiting")
    ...
    agent = create_deep_agent(
        ...
        subagents=[],
        middleware=[_ToolExclusionMiddleware(excluded=frozenset({"task"}))] if session.disable_task else (),
        checkpointer=session.checkpointer,
    )
    try:
        content = handoff_input(session, user_input) if session.handoff_pending else user_input
        ...
        if allow_compaction_retry and is_context_limit_error(e) and maybe_compact_session(session, event_bus=event_bus, event_layer=event_layer, event_data=event_data, force=True):
            return resume_agent_session(
                session, user_input, status_cb=status_cb, start=start, event_bus=event_bus, event_layer=event_layer,
                event_context=event_context, allow_compaction_retry=False,
            )
