...
- Use `.` for the workspace root when needed.
...
- When writing workspace artifacts, always use workspace-relative paths such as `{spec.output_path}`. Do not use `/` or absolute paths for stage deliverables.
...
"""
    ...
    def _validated_next_stage(self, spec, proposed: str) -> str:
        ...
        if spec.name == "experiment.loop" and proposed in {"experiment.analyze", "experiment.gate"} and not self._phase2_loop_ready():
            self._append_event("next_stage_rejected", stage=spec.name, next_stage=proposed, reason="experiment.loop requires at least one non-baseline optimization run before leaving the loop")
    ...
    def _resolved_next_stage(self, spec, proposed: str) -> str:
        ...
        if spec.name == "experiment.loop":
            next_stage = "experiment.analyze" if self._phase2_state()["results_ready"] and self._phase2_loop_ready() else "experiment.loop"
    ...
    def _phase2_state_text(self) -> str:
        ...
        lines.append(f"- loop ready for analyze: {'yes' if self._phase2_loop_ready(state) else 'no'}")
        return "\n".join(lines)

    def _phase2_loop_ready(self, state: dict[str, object] | None = None) -> bool:
        state = state or self._phase2_state()
        return bool(state.get("reproduced")) and int(state.get("runs", 0)) >= 2
