...
def _convert_virtual_paths(command: str, workspace_name: str) -> str:
    prefixes = (f"/{workspace_name}/", *_SYSTEM_PATH_PREFIXES)
    ...
    for prefix in prefixes:
        command = re.sub(rf"(?<!\S){re.escape(prefix)}[^\s;|&<>\'\"`]*", replace, command)
    return command
...
class WorkspaceBackend(LocalShellBackend):
    ...
    def _normalize_read_key(self, key: str, *, allow_dot: bool = False) -> tuple[str | None, str | None]:
        if key == ".":
            return (".", None) if allow_dot else (None, "Use a workspace-relative path, not '.'.")
        if key == "/":
            return None, "Use '.' for the workspace root, not '/'."
        if not os.path.isabs(key):
            return key, None
        return None, f"Absolute paths are not allowed: '{key}'. Use '.' for the workspace root or a workspace-relative path such as 'source_task/task.yaml'."

    def ls(self, path: str):
        path, error = self._normalize_read_key(path, allow_dot=True)
