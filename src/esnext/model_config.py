...
def invoke_plain_text(system_prompt: str, user_prompt: str, *, model: str = MODEL) -> str:
    msg = ChatOpenAI(
        model=model,
        base_url=BASE_URL,
        api_key=API_KEY,
        ...
        http_client=httpx.Client(trust_env=TRUST_ENV),
        **chat_provider_options(),
    ).invoke([SystemMessage(content=system_prompt), HumanMessage(content=user_prompt)])
