---
name: project-graph
description: Orchestrate milestone-based construction of a project or large subsystem. Use only when the LightCoder controller dispatches PROJECT_GRAPH.
---
...
- The controller must dispatch `PROJECT_GRAPH` in the `orchestration` flow with a unique attempt id.
...
1. Load the subgraph state or enter `INITIALIZE_PROJECT_STATE` if absent.
2. Run `PROJECT_LOOP_START` until completion, reroute, interruption, or external stop.
...
- Route guard applied: new flow -> `INITIALIZE_PROJECT_STATE`; resumed flow -> `PROJECT_LOOP_START`; new type evidence -> `ROUTE_TASK`; candidate complete -> `PHASE_3_FINALIZE`
- The proposed route is one of: `INITIALIZE_PROJECT_STATE`, `PROJECT_LOOP_START`, `ROUTE_TASK`, `PHASE_3_FINALIZE`.
