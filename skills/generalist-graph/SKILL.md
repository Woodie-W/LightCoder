---
name: generalist-graph
description: Orchestrate dynamic subgoal execution for mixed or unclassified coding tasks. Use only when the LightCoder controller dispatches GENERALIST_GRAPH.
---
...
- The controller must dispatch `GENERALIST_GRAPH` in the `orchestration` flow with a unique attempt id.
...
1. Load the subgraph state or enter `INITIALIZE_GENERALIST_STATE` if absent.
2. Run `GENERALIST_LOOP_START` until completion, reroute, interruption, or external stop.
...
- Route guard applied: new flow -> `INITIALIZE_GENERALIST_STATE`; resumed flow -> `GENERALIST_LOOP_START`; new type evidence -> `ROUTE_TASK`; candidate complete -> `PHASE_3_FINALIZE`
- The proposed route is one of: `INITIALIZE_GENERALIST_STATE`, `GENERALIST_LOOP_START`, `ROUTE_TASK`, `PHASE_3_FINALIZE`.
