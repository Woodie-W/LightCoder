---
name: select-next-subgoal
description: Select the next dependency-ready subgoal with the clearest verification path. Use only when the LightCoder controller dispatches SELECT_NEXT_SUBGOAL.
---
...
- The controller must dispatch `SELECT_NEXT_SUBGOAL` in the `generalist` flow with a unique attempt id.
- Resolve the declared inputs before work begins: `generalist.subgoals`, `generalist.outcomes`, `generalist.history`.
...
1. Filter incomplete and unblocked subgoals.
2. Prefer work that reduces uncertainty or unlocks multiple downstream goals.
3. Select one subgoal and define its immediate done condition.
4. If no valid subgoal exists, request decomposition revision.
...
- Proposed state updates are limited to: `generalist.active_subgoal`, `generalist.selection_reason`, `generalist.subgoal_oracle`.
...
- Route guard applied: valid local oracle -> `SELECT_SKILL`; no independently verifiable subgoal -> `REVISE_TASK_DECOMPOSITION`
- The proposed route is one of: `SELECT_SKILL`, `REVISE_TASK_DECOMPOSITION`.
- `progress.made_progress` is true only when the attempt added evidence, resolved an item, falsified a hypothesis, narrowed scope, or accepted a checkpoint.

## State Updates

- generalist.active_subgoal
- generalist.selection_reason
...
- Do not select a subgoal without an observable result.
