---
name: fix-build-and-dependencies
description: Resolve build-system or dependency blockers created or exposed by migration. Use only when the LightCoder controller dispatches FIX_BUILD_AND_DEPENDENCIES.
---
...
- The controller must dispatch `FIX_BUILD_AND_DEPENDENCIES` in the `transform` flow with a unique attempt id.
- Resolve the declared inputs before work begins: `transform.failure_diagnosis`, `transform.compatibility_matrix`, `workspace`.
...
1. Identify the first actionable build/dependency error.
2. Apply the smallest manifest, toolchain, generated-file, or configuration correction.
3. Re-run the narrow build check before broader tests.
4. Update the compatibility matrix and deprecated-item inventory.
...
- Proposed state updates are limited to: `transform.build_fix`, `transform.compatibility_matrix`, `workspace.current_revision`, `workspace.dirty_files`.
...
- Route guard applied: corrected candidate ready -> `VERIFY_BUILD_BEHAVIOR_COMPATIBILITY`; fix requires a new accepted step -> `TRANSFORM_LOOP_START`
...
- Do not indiscriminately upgrade unrelated dependencies.
