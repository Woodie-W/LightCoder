---
name: generalist-loop-start
description: Synchronize generalist state and begin one subgoal cycle. Use only when the LightCoder controller dispatches GENERALIST_LOOP_START.
---
...
- The controller must dispatch `GENERALIST_LOOP_START` in the `generalist` flow with a unique attempt id.
- Resolve the declared inputs before work begins: `generalist`, `workspace`, `external_run_config`.
...
1. Load the latest accepted revision and evidence.
2. Summarize completed, active, blocked, and uncertain subgoals.
3. Increment the cycle counter and select one next subgoal.
...
- Proposed state updates are limited to: `generalist.iteration`, `generalist.current_summary`, `generalist.last_revision`.
...
- Route guard applied: -> `SELECT_NEXT_SUBGOAL`
- The proposed route is one of: `SELECT_NEXT_SUBGOAL`.
- `progress.made_progress` is true only when the attempt added evidence, resolved an item, falsified a hypothesis, narrowed scope, or accepted a checkpoint.

## State Updates

- generalist.iteration
- generalist.current_summary
- generalist.last_revision
...
- Do not execute multiple unrelated subgoals in one cycle.
