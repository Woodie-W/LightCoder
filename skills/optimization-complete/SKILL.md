...
description: Seal the validated best optimization result for finalization. Use only when the LightCoder controller dispatches OPTIMIZATION_COMPLETE.
---
...
- The controller must dispatch `OPTIMIZATION_COMPLETE` in the `optimization` flow with a unique attempt id.
- Resolve the declared inputs before work begins: `optimization.final_evidence`, `optimization.best_result`, `optimization.history`.
...
1. Freeze the validated revision.
2. Summarize baseline, final metric, raw repeat count, correctness result, accepted changes, and limitations.
3. Mark the optimization subgraph complete.
...
- Proposed state updates are limited to: `optimization.status`, `optimization.final_summary`, `workspace.accepted_revision`.
...
- Do not make further tuning changes.
