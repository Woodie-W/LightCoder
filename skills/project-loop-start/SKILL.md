---
name: project-loop-start
description: Synchronize project state and begin one milestone cycle. Use only when the LightCoder controller dispatches PROJECT_LOOP_START.
---
...
- The controller must dispatch `PROJECT_LOOP_START` in the `project` flow with a unique attempt id.
- Resolve the declared inputs before work begins: `project`, `workspace`, `external_run_config`.
...
1. Load the latest checkpoint and accepted revision.
2. Summarize completed milestones, active risks, and current integration state.
3. Increment the cycle counter and choose the next milestone.
...
- Proposed state updates are limited to: `project.iteration`, `project.current_summary`, `project.last_revision`.
...
- Route guard applied: -> `SELECT_NEXT_MILESTONE`
- The proposed route is one of: `SELECT_NEXT_MILESTONE`.
...
- Do not reopen completed milestones without failure evidence.
