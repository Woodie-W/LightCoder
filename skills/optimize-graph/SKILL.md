---
name: optimize-graph
description: Orchestrate measured optimization with correctness preservation and rollback. Use only when the LightCoder controller dispatches OPTIMIZE_GRAPH.
---
...
- The controller must dispatch `OPTIMIZE_GRAPH` in the `orchestration` flow with a unique attempt id.
...
1. Load the subgraph state or enter `INITIALIZE_OPTIMIZATION_STATE` if absent.
2. Run `OPTIMIZATION_LOOP_START` until completion, reroute, interruption, or external stop.
...
- Route guard applied: new flow -> `INITIALIZE_OPTIMIZATION_STATE`; resumed flow -> `OPTIMIZATION_LOOP_START`; new type evidence -> `ROUTE_TASK`; candidate complete -> `PHASE_3_FINALIZE`
- The proposed route is one of: `INITIALIZE_OPTIMIZATION_STATE`, `OPTIMIZATION_LOOP_START`, `ROUTE_TASK`, `PHASE_3_FINALIZE`.
