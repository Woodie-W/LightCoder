---
name: accept-transformation-step
description: Promote a verified transformation step to the accepted state. Use only when the LightCoder controller dispatches ACCEPT_TRANSFORMATION_STEP.
---
...
- The controller must dispatch `ACCEPT_TRANSFORMATION_STEP` in the `transform` flow with a unique attempt id.
- Resolve the declared inputs before work begins: `transform.active_step`, `transform.candidate_revision`, `transform.last_verification`.
...
1. Confirm all checks required by the step oracle pass.
2. Commit or checkpoint the candidate revision.
3. Mark the step complete and update affected compatibility entries.
4. Record any deferred cleanup explicitly.
...
- Proposed state updates are limited to: `transform.accepted_steps`, `transform.steps`, `transform.current_revision`, `transform.deferred_cleanup`.
...
- Route guard applied: -> `UPDATE_TRANSFORM_STATE`
- The proposed route is one of: `UPDATE_TRANSFORM_STATE`.
...
- Do not accept a step with unexplained behavior drift.
