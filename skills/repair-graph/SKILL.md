---
name: repair-graph
description: Orchestrate the evidence-driven bug-repair loop. Use only when the LightCoder controller dispatches REPAIR_GRAPH.
---
...
- The controller must dispatch `REPAIR_GRAPH` in the `orchestration` flow with a unique attempt id.
...
1. Load the subgraph state or enter `INITIALIZE_REPAIR_STATE` if absent.
2. Run `REPAIR_LOOP_START` until completion, reroute, interruption, or external stop.
...
- Route guard applied: new flow -> `INITIALIZE_REPAIR_STATE`; resumed flow -> `REPAIR_LOOP_START`; new type evidence -> `ROUTE_TASK`; candidate complete -> `PHASE_3_FINALIZE`
- The proposed route is one of: `INITIALIZE_REPAIR_STATE`, `REPAIR_LOOP_START`, `ROUTE_TASK`, `PHASE_3_FINALIZE`.
