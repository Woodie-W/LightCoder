---
name: feature-graph
description: Orchestrate incremental feature implementation against explicit acceptance criteria. Use only when the LightCoder controller dispatches FEATURE_GRAPH.
---
...
- The controller must dispatch `FEATURE_GRAPH` in the `orchestration` flow with a unique attempt id.
...
1. Load the subgraph state or enter `INITIALIZE_FEATURE_STATE` if absent.
2. Run `FEATURE_LOOP_START` until completion, reroute, interruption, or external stop.
...
- Route guard applied: new flow -> `INITIALIZE_FEATURE_STATE`; resumed flow -> `FEATURE_LOOP_START`; new type evidence -> `ROUTE_TASK`; candidate complete -> `PHASE_3_FINALIZE`
- The proposed route is one of: `INITIALIZE_FEATURE_STATE`, `FEATURE_LOOP_START`, `ROUTE_TASK`, `PHASE_3_FINALIZE`.
